    ...
    private static void cargarPacientes(ClinicaService servicio) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                String[] p = linea.split(",", -1); // id,cedula,nombre,apellido,telefo
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(f))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                servicio.getTurnos().add(new Turno(
                        Integer.parseInt(p[0].trim()),pac,med,LocalDateTime.parse(p[4].trim(), FMT),EstadoTurno.valueOf(p[5].trim())));
            }
        } catch (IOException e) { System.out.println("Error: " + e.getMessage()); }
    }
    //  GUARDAR
